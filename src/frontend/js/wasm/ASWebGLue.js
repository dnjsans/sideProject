...
export function ASWebGLReady(wasmModule, importObject) {
  ...
  importObject.WebGL.__new = wasmModule.instance.exports["__new"];    //data를 wasm용 변환
  importObject.WebGL.__retain = wasmModule.instance.exports["__retain"];
  importObject.WebGL.RTTI_BASE = wasmModule.instance.exports["__rtti_base"];    //런타임 타입 인포메이션 ( 가상함수를 갖고 있는 클래스에서만 사용가능 )
}
...
export function initASWebGLue(importObject) {
  ...
  /**
   * uniform에 buffer할당을 위해서 포인터로 버퍼를 바로 가져온다.
   * @param {*} arr_ptr 
   * @returns  (typed)buffer
   */
  importObject.WebGL.getArrayView = (arr_ptr) => {    //포인터를 넘겨서 해당위치의 버퍼 가져오기 (***)
    const U32 = new Uint32Array(WebGL.memory.buffer);
    const id = U32[arr_ptr + WebGL.ID_OFFSET >>> 2];    //>>> 무조건int반환  unsigned 비트쉬프트 연산    /4와 같음  4바이트로 나눠서 계산. U32는 4바이트씩 들어 있음.

    const count = U32[WebGL.RTTI_BASE >>> 2];   //rtti 다음 4바이트는 count임
  ...
  importObject.WebGL.getArray = (arr) => {
    const input = WebGL.getArrayView(arr);
  ...
  importObject.WebGL.newString = (str) =>{
    if (str == null) return 0;
    const length = str.length;
    const ptr = WebGL.__new(length << 1, WebGL.STRING_ID);
    const U16 = new Uint16Array(WebGL.memory.buffer);
  ...
  importObject.WebGL.createContextFromCanvas = (canvas_id, context_type) => {
    try {
      console.log("createContextFromCanvas",canvas_id,WebGL.getString(canvas_id));
  ...
  */
  ...
  // specify a two-dimensional texture image
  const texImage2D_ = (ctx, target, level, internalformat, width, height, border, format, type, pixels) => {
    try {
      // THIS DOES NOT LOOK RIGHT TO ME
      //let tex = WebGL.getArrayView(pixels);
      //===============================
      //console.log("===================client>>>>>",target,level, internalformat, width, height, border,format, type);
      let tex = new Uint8Array(WebGL.getArrayView(pixels));
      //width = 3;
      //height = 3;
      //console.log("===================client>>>>>",target,level, WebGL.contextArray[ctx].LUMINANCE, width, height, border,WebGL.contextArray[ctx].LUMINANCE, type);
      
      WebGL.contextArray[ctx].texImage2D(target, level, internalformat, width, height, border, format, type, tex);
      //===============================
      //WebGL.contextArray[ctx].texImage2D(target, level, internalformat, width, height, border, format, type, tex);
    } catch (err) {
      console.log("texImage2D_ error");
      console.error(err);
    } // end catch
  }
  importObject.WebGL["texImage2D_<i32>"] = texImage2D_;
  importObject.WebGL["texImage2D_<u32>"] = texImage2D_;
  importObject.WebGL["texImage2D_<f32>"] = texImage2D_;
  ...
  importObject.WebGL.uniform2fv = (ctx, location, v) => {
    try {
      // getUniformLocation: vec2 , getArrayView : string uniform변수명
